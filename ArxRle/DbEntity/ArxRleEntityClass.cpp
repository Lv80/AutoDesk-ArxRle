...
void CLayer::SetName(ArxRleString lpszName)
{
	if(m_pLayer != NULL)
	{
		m_pLayer->setName(AcString(lpszName));
	}
}
...
{
	if(m_pLayer != NULL)
		m_pLayer->setDescription(AcString(lpszDescription));
}
...
void CTextStyle::SetName(ArxRleString lpszName)
{
	...
	{
		m_pTextStyleRecord->setName(AcString(lpszName));
	}
}
...
void CTextStyle::SetFont(ArxRleString lpszFontName, bool bold, bool italic)
{
	try
	{
		if(m_pTextStyleRecord != NULL && lpszFontName.GetLength() > 0)
		{
			try
			{
				m_pTextStyleRecord->setFont(AcString(lpszFontName), bold, italic, 134, 0);
			}
			catch(...)
			{
			}
...
void CTextStyle::SetFont(ArxRleString lpszFontName, ArxRleString lpszBigFontName, bool vertical/* = false*/)
{
	try
	{
		if(m_pTextStyleRecord != NULL && lpszFontName.GetLength() > 0)
		{
			try
			{
				m_pTextStyleRecord->setFileName(AcString(lpszFontName));
				if(lpszBigFontName.GetLength() > 0)
					m_pTextStyleRecord->setBigFontFileName(AcString(lpszBigFontName));
...
void CTextStyle::GetFont(char* strFontFace, bool& bBold, bool& bItalic)
{
	...
	{
		ACHAR* oFace;
		int charset, pitchfamily;
		Adesk::Boolean blnBold = bBold, blnItalic = bItalic;
		m_pTextStyleRecord->font(oFace, blnBold, blnItalic, charset, pitchfamily);
...
int CTextStyle::GetFontType() const
{
	if(m_pTextStyleRecord != NULL)
	{
		...
		int nCharset, nPitch;
		m_pTextStyleRecord->font(oFace, bBold, bItacil, nCharset, nPitch);
...
void CTableStyle::SetName(ArxRleString lpszName)
{
	...
	{
		m_pTableStyle->setName(AcString(lpszName));
	}
}
...
void CTableStyle::CreateCellStyle(ArxRleString lpszName)
{
	if(m_pTableStyle != NULL)
	{
		m_pTableStyle->createCellStyle(AcString(lpszName));
	}
}
...
void CTableStyle::SetRotation(double fRotation, ArxRleString lpszCellType)
{
	if(m_pTableStyle != NULL)
	{
		if(lpszCellType.GetLength() > 0)
			m_pTableStyle->setRotation(fRotation, AcString(lpszCellType));
		else
		{
...
void CTableStyle::SetMargin(ArxMargin eMargin, double fMargin, ArxRleString lpszCellStyle)
{
	if(m_pTableStyle != NULL)
	{
		if(lpszCellStyle.GetLength() > 0)
			m_pTableStyle->setMargin((AcDb::CellMargin)eMargin, fMargin, AcString(lpszCellStyle));
		else
		{
			AcStringArray AcStrArray;
			m_pTableStyle->getCellStyles(AcStrArray);
...
double CTableStyle::GetRotation(ArxRleString lpszCellStyle)
{
	...
	{
		return m_pTableStyle->rotation(AcString(lpszCellStyle));
	}
...
void CBlockRecord::SetName(ArxRleString lpszName)
{
	if(!m_pBlockRecord->objectId().isNull() && lpszName.GetLength() > 0)
		m_pBlockRecord->setName(AcString(lpszName));
}
...
bool CBlockRecord::LoadFile(ArxRleString lpszPathName)
{
	if(!m_pBlockRecord->objectId().isNull())
	{
		...
		{
			m_pBlockRecord->setPathName(AcString(lpszPathName));
			return true;
		}
	}
...
void CBlockRecord::SetComments(ArxRleString lpszComment)
{
	if(!m_pBlockRecord->objectId().isNull())
	{
		m_pBlockRecord->setComments(AcString(lpszComment));
	}
}
...
void CDimensionStyle::SetName(ArxRleString lpszName)
{
	if(!m_pDimensionStyle->objectId().isNull())
	{
		m_pDimensionStyle->setName(AcString(lpszName));
	}
}
...
void CDimensionStyle::SetArrow(ArxRleString lpszName)		//箭头样式
{
	if(!m_pDimensionStyle->objectId().isNull())
	{
		...
		m_pDimensionStyle->setDimblk(AcString(lpszName));
	}
}
...
void CDimensionStyle::SetArrow(ArxRleString lpszName1, ArxRleString lpszName2)		//箭头1样式,箭头2样式
{
	if(!m_pDimensionStyle->objectId().isNull())
	{
		...
		if(lpszName1.GetLength() > 0)
			m_pDimensionStyle->setDimblk1(AcString(lpszName1));
		if(lpszName2.GetLength() > 0)
			m_pDimensionStyle->setDimblk2(AcString(lpszName2));
	}
}
...
void CUcsRecord::SetName(ArxRleString lpszName)
{
	if(!m_pUcsRecord->objectId().isNull())
	{
		m_pUcsRecord->setName(AcString(lpszName));
	}
}
...
void CMText::SetText(ArxRleString lpszText)
{
	if(!isNull())
	{
		...
		else pEntity->setContents(AcString(lpszText));
	}
}
	...
	try
	{
		if(!isNull())
		{
			...
			for(pIterator->start(); !pIterator->done(); pIterator->step())
			{
				...
				if(pAtt != NULL && !pAtt->isConstant())
				{
					if(pAtt->tag() == AcString(lpszName))
					{
						pAtt->upgradeOpen();
						pAtt->setTextString(AcString(lpszText));
						pAtt->downgradeOpen();
					}
				}
			}
	...
	try
	{
		if(!isNull())
		{
			...
			for(pIterator->start(); !pIterator->done(); pIterator->step())
			{
				...
				if(pAtt != NULL && !pAtt->isConstant())
				{
					if(pAtt->tag() == AcString(lpszName) == 0)
					{
						pAtt->upgradeOpen();
...
void CDimension::SetText(ArxRleString lpszText)
{
	...
	{
		AcDbRotatedDimension* pEntity = (AcDbRotatedDimension*)m_pEntity;
		pEntity->setDimensionText(AcString(lpszText));
	}
}
...
void CDimension::SetArrow(ArxRleString lpszName)
{
	if(!m_pEntity->objectId().isNull())
	{
		...
		pDimRecord->setDimblk1(AcString(lpszName));
		pDimRecord->setDimblk2(AcString(lpszName));
		pDimRecord->close();
	}
}
...
void CHatch::SetGradiant(ArxRleString lpszName, bool bUser)
{
	...
	{
		AcDbHatch* pEntity = (AcDbHatch*)m_pEntity;
		pEntity->setGradient(bUser ? AcDbHatch::kUserDefinedGradient : AcDbHatch::kPreDefinedGradient, AcString(lpszName));
	}
}
...
bool CHatch::SetPattern(ArxRleString lpszName, ArxHatchPatternType eType)
{
	try
	{
		...
		{
			AcDbHatch* pEntity = (AcDbHatch*)m_pEntity;
			pEntity->setPattern((AcDbHatch::HatchPatternType)eType, AcString(lpszName));
			return true;
		}
	}
	catch(...)
...
void CTable::SetColName(long nCol, ArxRleString lpszName)
{
	...
	{
		AcDbTable* pEntity = (AcDbTable*)m_pEntity;
		pEntity->setColumnName(nCol, AcString(lpszName));
	}
}
...
void CLayout::SetPlotPaperSize(ArxRleString lpszName)
{
    if(!m_pLayout->objectId().isNull())
    {
	    try
	    {
		    ...
		    pValidator->setCanonicalMediaName(m_pLayout, AcString(lpszName));
...
{
	if(!m_pLayout->objectId().isNull())
		m_pLayout->setLayoutName(AcString(lpszName));
}
...
void CGroup::SetDescription(ArxRleString lpszName)
{
	if(IsNull())
	{
		AcString odstrName(lpszName);
		m_pGroup->setDescription(odstrName);
	}
}
...
void CGroup::SetLinetype(ArxRleString lpszName)
{
	...
	{
		AcString odstrName(lpszName);
		m_pGroup->setLinetype(odstrName);
	}
}
...
void CGroup::SetMaterial(ArxRleString lpszName)
{
	...
	{
		AcString odstrName(lpszName);
		m_pGroup->setMaterial(odstrName);
	}
}
...
AcDbObjectId AddLayout(ArxRleString lpszName)
{
	try
	{
		AcString AcName(lpszName);
		AcApLayoutManager *pLayMan = (AcApLayoutManager*)acdbHostApplicationServices()->layoutManager();
		AcDbLayout *pLayout = pLayMan->findLayoutNamed(AcName,true);
		AcDbObjectId objectId = pLayout->objectId();
		if(objectId.isValid())
			return objectId;

		pLayMan->createLayout(AcName, objectId, ARXRLEDS->objectId(),NULL);
		if(objectId.isValid())
			return objectId;
	}
	catch(...)
...
bool DelLayout(ArxRleString lpszName)
{
	try
	{
		if(lpszName.GetLength() > 0)
		{
			AcString AcName(lpszName);
			AcApLayoutManager *pLayMan = (AcApLayoutManager*)acdbHostApplicationServices()->layoutManager();
			if(pLayMan->findLayoutNamed(AcName,true)->objectId().isValid())
			{
				...
				if(objectId.isValid())
				{
					...
					if(szName == AcName)
					{
						...
						objectId = ARXRLEDB->currentSpaceId();
						AcDbObject* obj;
						acdbOpenAcDbObject(obj, objectId, AcDb::kForRead);
					}
				}
				pLayMan->deleteLayout(AcName);
				return true;
			}
		}
	}
...
void RenameLayout(ArxRleString lpszOldName, ArxRleString lpszNewName)
{
	try
	{
		...
		{
			AcString AcOldName(lpszOldName);
			AcApLayoutManager *pLayMan = (AcApLayoutManager*)acdbHostApplicationServices()->layoutManager();
			if(pLayMan->findLayoutNamed(AcOldName,true)->objectId().isValid())
				pLayMan->renameLayout(AcOldName, AcString(lpszNewName));
		}
	}
	catch(...)
	{
...
AcDbObjectId SetCurLayout(ArxRleString lpszName)
{
	try
	{
		if(lpszName.GetLength() > 0)
		{
			...
			AcDbLayout* pLayout = pLayMan->findLayoutNamed(AcName,TRUE);
			if(pLayout->objectId().isValid())
				return pLayout->objectId();
		}
		else
		{
			AcApLayoutManager *pLayMan = (AcApLayoutManager*)acdbHostApplicationServices()->layoutManager();
			AcDbLayout* pLayout = pLayMan->findLayoutNamed(AcString(lpszName),true);
			if(pLayout->objectId().isValid())
				return pLayout->objectId();
		}
	}
...
AcDbObjectId GetLayoutOfModelSpace()
{
	...
	{
		AcDbObjectId objectId = ARXRLEDS->getLayoutId();
		AcApLayoutManager *pLayMan = (AcApLayoutManager*)acdbHostApplicationServices()->layoutManager();
		AcDbLayout* pLayout = pLayMan->findLayoutNamed(pLayMan->findActiveLayout(TRUE),TRUE);
		if(!pLayout->objectId().isNull())
		{
			pLayout->setBlockTableRecordId(objectId);
			ACHAR* AcName;
			pLayout->getLayoutName(AcName);
			if(!pLayout->objectId().isNull())
...
AcDbObjectId AddBlockRecord(ArxRleString lpszName)
{
    ...
    try
    {
		try
		{
			...
			if(!pBlockTable->has(AcString(lpszName)))
				ARXRLEDB->insert(objectId, AcString(lpszName), AcString(lpszName),ARXRLEDB, false);
			else
				pBlockTable->getAt(AcString(lpszName),objectId);
    ...
    try
    {
		if(lpszFileName.GetLength() > 0)
		{
			if(PathFileExists(lpszFileName))
			{
				...
				if(!pBlockTable->has(AcString(szName)))
				{
					AcDbObjectId xrefBlkId;
					acdbAttachXref(ARXRLEDB, AcString(lpszFileName),  AcString(szName), xrefBlkId);
					pBlockTableRcd = (AcDbBlockTableRecord*)GetRecordPtr(xrefBlkId);
				}
				else
				{
					AcDbObjectId objectId;	
					pBlockTable->getAt(AcString(szName),objectId);
...
bool GetBlockRecordName(ArxRleString lpszFileName, CStringArray& arrName)
{
	try
	{
		...
		{
			AcDbDatabase* pDatabase = new AcDbDatabase(false,true);
			pDatabase->readDwgFile(AcString(lpszFileName));
			AcDbBlockTable* pBlockTable;
			pDatabase->getBlockTable(pBlockTable, AcDb::kForRead);
...
bool SetCurLineType(ArxRleString lpszName)
{
	try
	{
		if(lpszName.GetLength() > 0)
		{
			...
			if(pLinetypeTable->has(AcString(lpszName)))
			{
				AcDbObjectId objectId;
				pLinetypeTable->getAt(AcString(lpszName), objectId);
...
AcDbObjectId SetCurrentUcs(ArxRleString lpszName)
{
	...
	AcDbUCSTable* pUcsTable;
	ARXRLEDB->getUCSTable(pUcsTable, AcDb::kForWrite);
	if(pUcsTable->has(AcString(lpszName)))
	{
		pUcsTable->getAt(lpszName, objectId);
		ARXRLEDB->setUcsBase(objectId);
	}
...
AcDbBlockTableRecord* ArxRleDrawSpace()
{
	...
	if(!pLayout->objectId().isNull())
	{
		AcDbObjectId objectId = pLayout->getBlockTableRecordId();
		ACHAR* AcName;
		pLayout->getLayoutName(AcName);
		if(!pLayout->objectId().isNull())
			pLayMan->setCurrentLayout(AcName);
...
AcDbBlockTableRecord* GetDrawSpace(ArxSpaceType SpaceType)
{
	...
	case ArxModel:
		{
			AcDbLayout* pLayout = pLayMan->findLayoutNamed(AcString("*Model_Space"),TRUE);
			if(!pLayout->objectId().isNull())
			{
				AcDbObjectId objectId = pLayout->getBlockTableRecordId();
				pSpace = (AcDbBlockTableRecord*)GetRecordPtr(objectId);
			}
	...
	case ArxPaper:
		{
			AcDbLayout* pLayout = pLayMan->findLayoutNamed(AcString("*Paper_Space"),TRUE);
			if(!pLayout->objectId().isNull())
			{
				AcDbObjectId objectId = pLayout->getBlockTableRecordId();
				pSpace = (AcDbBlockTableRecord*)GetRecordPtr(objectId);
	...
	case ArxActive:
		{
			...
			AcDbLayout* pLayout = pLayMan->findLayoutNamed(szName,TRUE);
			if(!pLayout->objectId().isNull())
			{
...
void AddMText(ArxRleString lpszText, CPt3d& oInsertPt, double fWidth, double fHeight, double fRadiusotation, ArxAlignment eAlign)
{
	...
	if(abs(fWidth) > 0)
		...
		pMText->setWidth(fabs(lpszText.GetLength())); //文本范围宽度
