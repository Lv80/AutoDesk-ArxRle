...
void ArxRleUiTdcSqxElement::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, ARXRLE_SQX_ELEMENT_EB_S, m_S);
...
void ArxRleUiTdcSqxElement::OnBnClickedSqxElementBnImport()
{
	...
	if (dlg.DoModal() == IDOK)
	{
		...
		if(m_curveElement.m_nSQXNum > 0)
		{
			m_sqx = m_curveElement.m_vecSQXElement[0];
			m_S = m_sqx.m_S;
			m_H = m_sqx.m_H;
			m_R = m_sqx.m_R;
			m_I1 = m_sqx.m_I1*1000;
			m_I2 = m_sqx.m_I2*1000;
			m_L = m_sqx.m_L;
			m_T = m_sqx.m_T;
			m_E = m_sqx.m_E;
		}
...
void ArxRleUiTdcSqxElement::OnBnClickedSqxElementBnCalc() 
{
	...
	if(m_openPathName.IsEmpty())
	{
		...
		m_sqx.m_H = m_H;
		m_sqx.m_R = m_R;
		m_sqx.m_I1 = m_I1/1000;
		m_sqx.m_I2 = m_I2/1000;
		m_sqx.m_L = fabs(m_sqx.m_R * (m_sqx.m_I1 - m_sqx.m_I2));
		m_sqx.m_T = m_sqx.m_L / 2;
		if(m_sqx.m_R != 0) m_sqx.m_E = m_sqx.m_T * m_sqx.m_T / 2 / m_sqx.m_R;
	}
